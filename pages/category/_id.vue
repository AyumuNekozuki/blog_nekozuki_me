...
export default {
  async asyncData({ $axios, params }) {
    let [category_archive_datas, category_datas, newest_datas] = await Promise.all([
      $axios.$get(`/api_mc_nekolog/v1/article?filters=category%5Bequals%5D${params.id}&limit=500&orders=-published_At`),
      $axios.$get(`/api_mc_nekolog/v1/category?filters=id%5Bequals%5D${params.id}&limit=100&orders=-published_At`),
